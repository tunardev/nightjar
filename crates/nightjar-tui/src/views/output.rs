...
/// # Errors
/// fails if the file cannot be read for any reason but being missing
pub fn load_output(path: Option<&Path>) -> Result<OutputContent> {
    ...
    let read_result = if len <= VIEW_TAIL_THRESHOLD {
        ...
        read_tail(path, VIEW_TAIL_THRESHOLD).map(|bytes| {
            let label = format!("showing last 1MB - full file at {}", path.display());
            (bytes, Some(label))
        })
    };
...
fn render_content(frame: &mut Frame, area: Rect, content: &OutputContent, scroll: usize) {
    ...
    match content {
        ...
        OutputContent::Loaded(loaded) => {
            let banner_rows = usize::from(loaded.label.is_some());
            let lines: Vec<Line> = loaded
                .display_lines()
                .into_iter()
                .enumerate()
                .map(|(row, text)| {
                    let style = if row < banner_rows {
                        Style::default()
                            .fg(Color::Yellow)
                            .add_modifier(Modifier::ITALIC)
                    } else if text == trimmed_marker {
                        Style::default()
                            .fg(Color::Black)
                            .bg(Color::Yellow)
...
mod tests {
    ...
    #[test]
    fn find_next_match_survives_the_scroll_position_g_leaves_behind() {
        let lines = ["alpha", "beta error", "gamma"];
        assert_eq!(
            find_next_match(&lines, "error", usize::MAX),
            Some(1),
            "pressing G parks the scroll at usize::MAX; searching from there must still find a line"
        );
    }
