...
impl SortMode {
    const fn next(self) -> Self {
        match self {
            Self::Name => Self::ProblemsFirst,
            Self::ProblemsFirst => Self::LastRun,
...
impl App {
    ...
    pub const fn set_output_scroll(&mut self, line: usize) {
        ...
        match key.code {
            ...
            KeyCode::Char('x') => match self.selected_run() {
                Some(run) if run.status == RunStatus::Running => {
                    run.pid.map_or(Action::None, |pid| Action::Kill {
                        job: job.to_string(),
                        run_id: run.id.clone(),
                        pid,
                    })
...
const fn problem_rank(job: &JobRow) -> u8 {
