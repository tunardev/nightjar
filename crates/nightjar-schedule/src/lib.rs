...
#[derive(Debug, Clone)]
pub struct Schedule {
    firings: jiff_cron::Schedule,
    source: String,
}

impl Schedule {
    /// # Errors
    /// fails on `@reboot`, an unknown `@` shortcut, or a schedule neither grammar accepts
    pub fn parse(input: &str) -> Result<Self> {
        let cron_expr = match human::to_cron(input) {
        ...
        let normalized = cron::normalize_cron(&cron_expr)
            .with_context(|| format!("invalid schedule {input:?}"))?;
        let firings = jiff_cron::Schedule::from_str(&normalized)
            .with_context(|| format!("invalid schedule {input:?}"))?;
        Ok(Self {
            firings,
    ...
    /// # Errors
    /// never fails: stepping an already-parsed schedule forward cannot fail
