...
pub struct FakeSpawner {
    ...
    pids: Mutex<Vec<u32>>,
    start_lag_ms: AtomicI64,
    db_path: std::path::PathBuf,
    clock: Arc<dyn Clock>,
}

impl FakeSpawner {
    pub fn new(fail: bool, db_path: std::path::PathBuf, clock: Arc<dyn Clock>) -> Arc<Self> {
        Arc::new(Self {
    ...
    pub fn failing_only(
        ...
        let s = Self::new(false, db_path, clock);
    ...
    pub fn records_start_late_by_ms(&self, lag: i64) {
        self.start_lag_ms.store(lag, Ordering::SeqCst);
    }
}
    ...
    fn spawn(&self, job: &str, run_id: &str, trigger: Trigger) -> anyhow::Result<Child> {
        ...
        *self
        ...
        let fail = self
            ...
            .unwrap()
            .as_ref()
            .map_or_else(|| self.fail.load(Ordering::SeqCst), |only| only == job);
        if fail {
        ...
        let started_at = self.clock.now()
            + jiff::Span::new().milliseconds(self.start_lag_ms.load(Ordering::SeqCst));
        store
            .start_run(run_id, job, trigger, started_at, &stub, &stub)
...
impl AbandoningSpawner {
    pub fn new(db_path: std::path::PathBuf, clock: Arc<dyn Clock>) -> Arc<Self> {
        Arc::new(Self {
            attempts: AtomicUsize::new(0),
            db_path,
            clock,
...
pub struct UnrecordedSpawner {
    per_job: Mutex<std::collections::HashMap<String, usize>>,
}

impl UnrecordedSpawner {
    pub fn new() -> Arc<Self> {
        Arc::new(Self {
            per_job: Mutex::new(std::collections::HashMap::new()),
        })
...
impl Spawner for UnrecordedSpawner {
    fn spawn(&self, job: &str, _run_id: &str, _trigger: Trigger) -> anyhow::Result<Child> {
        *self
            .per_job
            .lock()
            .unwrap()
