...
fn disabled_jobs_queue_is_not_drained() {
    ...
    let spawner = FakeSpawner::new(false, paths.db_path.clone(), clock.clone());
    let mut d = Daemon::with_spawner(paths, clock, spawner.clone()).unwrap();
    d.tick().unwrap();
...
fn jobs_directory_surfaces_as_error_not_silently_empty_when_it_is_unreadable() {
    ...
    let clock = Arc::new(FixedClock::new("2026-06-01T00:00:00Z".parse().unwrap()));
    let mut d = Daemon::new(paths.clone(), clock).unwrap();
    assert!(d.tick().unwrap().is_empty());
...
fn trigger_already_honoured_is_not_recorded_missed_on_restart() {
    ...
    let mut d2 = Daemon::with_spawner(paths, clock, spawner2.clone()).unwrap();
    d2.tick().unwrap();
...
#[test]
fn overlap_skip_does_not_start_second_run_while_first_has_not_recorded_itself_yet() {
    let (_t, paths) = setup(&[(
        "beat",
        "command = \"true\"\nschedule = \"* * * * * *\"\noverlap = \"skip\"\n",
    )]);
    let clock = Arc::new(FixedClock::new("2026-06-01T00:00:00Z".parse().unwrap()));
    let spawner = UnrecordedSpawner::new();
    let mut d = Daemon::with_spawner(paths, clock.clone(), spawner.clone()).unwrap();
    ...
    assert_eq!(
        spawner.count_for("beat"),
        1,
        "a run this daemon forked is in flight from the fork onwards, not from \
         whenever its wrapper gets round to writing the row"
    );
}
