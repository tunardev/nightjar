...
fn consumed_gap_is_not_replayed_by_daemon_that_starts_next() {
    ...
    let (mut second, second_spawns) = daemon_with_counting_spawner(paths.clone(), clock);
    second.tick().unwrap();
...
#[test]
fn job_is_not_caught_up_for_occurrences_this_daemon_already_ran_it_for() {
    let (_t, paths) = setup(&[
        (
            "broken",
            "command = \"true\"\nschedule = \"every 1 minute\"\n",
    ...
    spawner.records_start_late_by_ms(40);
    ...
    assert_eq!(
        spawner.count_for("healthy"),
        4,
        "one job failing every tick holds the store's watermark back; that is not \
         an outage, and must not replay minutes this daemon already ran"
    );
}
