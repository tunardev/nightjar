...
fn host_outcome(outcome: RunOutcome) -> HostOutcome {
    match outcome {
        ...
        for _ in 0..worker_count {
            ...
            scope.spawn(move || {
                loop {
                    ...
                    let outcome = host_outcome(runner.run(&host, args));
...
mod tests {
    ...
    #[test]
    fn wedged_host_is_unreachable_even_when_it_exits_zero_on_the_kill_signal() {
        let outcome = run_command_with_timeout(
            "sh",
            &[
                "-c".to_string(),
                "trap 'exit 0' TERM; printf partial; sleep 30".to_string(),
            ],
        ...
        assert_eq!(
            outcome,
            RunOutcome::ConnectFailed,
            "nightjar killed this run, so the exit status describes the kill, not a remote result; \
             reporting it as a clean exit turns a timed-out host into a success with no output"
        );
    }
