...
#[test]
fn superseding_a_placeholder_does_not_leave_its_message_attached_to_the_make_up_run() {
    let store = Store::open_in_memory().unwrap();
    let occurrence: Timestamp = "2026-06-01T00:15:00Z".parse().unwrap();
    let started: Timestamp = "2026-06-01T02:00:00Z".parse().unwrap();
    ...
    store
        ...
        .unwrap();
    store
        .set_run_message("r1", "the daemon was down for this occurrence")
        .unwrap();
    ...
    assert_eq!(
        store.get_run("r1").unwrap().unwrap().message,
        None,
        "the reason the occurrence was missed says nothing about the run now under way"
    );
}
