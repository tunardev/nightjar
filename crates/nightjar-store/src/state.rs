...
impl Store {
    /// # Errors
    /// fails if the write to `job_state` is blocked past `busy_timeout`
    ...
    /// # Errors
    /// fails if the `job_state` row cannot be read or does not decode
    ...
    /// # Errors
    /// fails if the `job_state` table cannot be read, or any row does not decode
    ...
    /// # Errors
    /// fails if the delete is blocked past `busy_timeout`
