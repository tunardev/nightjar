...
use nightjar_core::format::{exit_reason, relative_time};
...
pub trait CommandRunner {
    /// # Errors
    /// fails if the command cannot be started, or exits non-zero
...
impl CommandRunner for RealRunner {
    fn run(&self, argv: &[String]) -> Result<()> {
        let [prog, rest @ ..] = argv else {
            bail!("nothing to run: the command is empty");
        };
        ...
        if !status.success() {
            bail!("`{}` ended with {}", argv.join(" "), exit_reason(&status));
        }
        Ok(())
    }
}
...
/// # Errors
/// fails if no unit can be planned for this platform, or it cannot be written
...
/// # Errors
/// fails if no unit can be planned for this platform, or it cannot be removed
...
/// # Errors
/// fails if no unit can be planned for this platform
...
fn do_install(
    ...
    println!("it starts `{} daemon` at login", exe.display());

    if dry_run {
        println!("dry run: not registering with the OS ({DRY_RUN_ENV} is set)");
    } else {
        runner.run(&plan.register)?;
        println!("registered; jobs will fire from now on, and again after a reboot");
    }
...
fn maybe_enable_linger(is_linux: bool, dry_run: bool, runner: &dyn CommandRunner) {
    ...
    let Some(user) = std::env::var("USER").ok().filter(|u| !u.is_empty()) else {
        eprintln!(
            "nightjar: $USER is not set, so lingering could not be enabled; run \
             `loginctl enable-linger <user>` yourself, or the daemon stops at logout"
        );
        return;
    };
    let argv = vec![
    ...
    if let Err(e) = runner.run(&argv) {
        eprintln!(
            "nightjar: lingering could not be enabled ({e:#}); the service is \
             installed, but stops at logout until you run \
...
fn do_uninstall(
    ...
    if !plan.unit_path.exists() {
        println!(
            "nothing installed at {}; nothing to remove",
    ...
    println!(
        "removed {}; nothing will start nightjar at login any more",
        plan.unit_path.display()
    );
    Ok(0)
}
...
fn status_line(paths: &Paths, exe: &Path, root: &Path) -> Result<String> {
    ...
    Ok(match daemon_health(&store, now)? {
        DaemonHealth::Responding(beat) => format!(
            "service installed at {}; daemon running (pid {})",
            plan.unit_path.display(),
            beat.pid
        ),
        DaemonHealth::NotResponding(beat) => format!(
            "service installed at {}; daemon not running, last heartbeat {}",
            plan.unit_path.display(),
            relative_time(beat.at, now)
        ),
        DaemonHealth::NeverRun => format!(
