...
pub fn refuse_as_not_found(message: &str, json: bool) -> Result<i32> {
    if json {
        println!(
            "{}",
            serde_json::json!({ "schema": SCHEMA_VERSION, "error": message })
        );
        return Ok(1);
    }
    bail!("{message}")
...
pub fn skew_warning(host: &str, value: &Value) -> Option<String> {
...
pub fn collect(results: Vec<HostResult>) -> Vec<HostView> {
    results
        .into_iter()
        .map(|r| {
...
pub const fn problem_label(payload: &HostPayload) -> Option<&'static str> {
    match payload {
...
pub fn any_problem(views: &[HostView]) -> bool {
    views
        .iter()
...
pub fn merged_json(views: &[HostView]) -> String {
...
mod tests {
    ...
    fn host_classifies_as_ok_with_no_problem_and_no_warning_when_it_is_healthy() {
        let outcome =
            HostOutcome::Success(format!(r#"{{"schema":{SCHEMA_VERSION},"jobs":[]}}"#), 0);
