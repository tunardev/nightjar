...
use nightjar_core::guidance::{job_already_exists, nothing_fires_until_the_daemon_runs};
use nightjar_core::paths::{Paths, validate_job_name};
use nightjar_schedule::Schedule;
use toml_edit::{DocumentMut, value};

/// # Errors
/// fails if the job name is unusable, a job file already exists, or the write fails
pub fn cmd_add(
    name: &str,
    cmd: &str,
    at: &str,
    ...
    if path.exists() {
        bail!("{}", job_already_exists(name, &path));
    }
    ...
    println!("{}", nothing_fires_until_the_daemon_runs(name));
    Ok(0)
}
