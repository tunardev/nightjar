...
fn status_reports_exit_state_after_runs() {
    ...
    assert!(
        broken.split_whitespace().any(|cell| cell == "-"),
...
#[test]
fn logs_says_what_happened_when_a_failed_run_captured_nothing() {
    let tmp = setup();
    nj(tmp.path(), &["run", "beta"]);

    let out = nj(tmp.path(), &["logs", "beta"]);
    let told = String::from_utf8_lossy(&out.stderr);
    assert!(told.contains("beta failed with exit 4"), "got:\n{told}");
    assert!(told.contains("printed nothing"), "got:\n{told}");
}

#[test]
fn logs_keeps_stdout_to_exactly_what_the_job_printed() {
    let tmp = setup();
    nj(tmp.path(), &["run", "alpha"]);
    ...
    assert_eq!(
        ...
        "alpha-out\n",
        "everything nightjar has to say belongs on stderr, so the log stays pipeable"
    );
    assert!(
        String::from_utf8_lossy(&out.stderr).contains("alpha succeeded"),
        "stderr: {}",
...
#[test]
fn logs_exits_nonzero_and_names_the_job_when_no_such_job_exists() {
    let tmp = setup();

    let out = nj(tmp.path(), &["logs", "ghost"]);
    assert!(
        !out.status.success(),
        "a typo in the job name is a mistake, not an empty report"
    );
    assert!(
    ...
    let out = nj(tmp.path(), &["logs", "ghost", "--json"]);
    assert!(!out.status.success());
    ...
    assert!(
        out.status.success(),
        "a job that exists but has never run is still not an error"
    );
}

#[test]
fn logs_still_replays_history_when_job_file_was_deleted_after_it_ran() {
    let tmp = setup();
    nj(tmp.path(), &["run", "alpha"]);
    std::fs::remove_file(tmp.path().join("jobs/alpha.toml")).unwrap();

    let out = nj(tmp.path(), &["logs", "alpha"]);
    assert!(
        out.status.success(),
        "recorded history outlives the job file; stderr: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    assert!(String::from_utf8_lossy(&out.stdout).contains("alpha-out"));
}
