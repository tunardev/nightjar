...
#[test]
fn status_exits_nonzero_when_the_filter_matches_nothing_and_no_jobs_directory_exists_yet() {
    let tmp = tempfile::tempdir().unwrap();

    let out = nj(tmp.path(), &["status", "ghost"]);
    assert!(
        !out.status.success(),
        "a fresh machine must not report an unknown job as a clean, empty status"
    );
    assert!(String::from_utf8_lossy(&out.stderr).contains("ghost"));
    ...
    let unfiltered = nj(tmp.path(), &["status"]);
    assert!(
        unfiltered.status.success(),
        "an unfiltered status on a fresh machine is still not an error"
    );
    assert!(
        String::from_utf8_lossy(&unfiltered.stdout).contains("nightjar add"),
        "and it must still name the command that creates one"
    );
}
