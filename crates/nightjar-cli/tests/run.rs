...
fn run_refuses_when_overlap_is_skip_or_queue_and_run_is_already_in_flight() {
    for (job, policy) in [("skipjob", "skip"), ("queuejob", "queue")] {
        let tmp = setup(
            job,
        ...
        let err = String::from_utf8_lossy(&out.stderr);
        assert!(
            err.contains(job) && err.contains(policy),
            "stderr must name the job and spell the policy the way the job file does; \
             stderr was: {err}"
        );
        assert_eq!(
